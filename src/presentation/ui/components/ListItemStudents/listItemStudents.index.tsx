  ...
  return (
    ...
    <TouchableOpacity
      ...
      <View className="items-center justify-center" >
        ...
        {data?.name && <Image
          className="rounded-full aspect-square h-[70px] w-[70px] bg-red-200 mr-4  "
      ...
      <View className=" flex-col py-4  w-full shrink justify-between "  >
        {data?.name && <Text numberOfLines={2} className=" font-karla700Bold text-lg text-neutral-300 leading-[20px] " >{data?.name?.first} {data?.name?.last}</Text>}


        <View className=" flex-row  justify-between w-full " >

          {data?.name && <Text numberOfLines={2} className="font-karla500Medium text-sm text-neutral-300  capitalize" >
            {convertGender(data?.gender)}</Text>}

          {data?.name && <Text numberOfLines={2} className="font-karla500Medium text-sm text-neutral-300" >{format(data?.dob?.date, 'dd/MM/yyyy')}</Text>}
